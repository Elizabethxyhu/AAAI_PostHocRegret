...
def IPOfunc(A =None,b =None,G=None,c=None,hTrue=None,alpha0=0.9995,beta=0.1,pc = True,
    ...
    class IPOfunc_cls(Function):        
        ...
        def forward(ctx,h):
            ...
            if not complete:
                ...
                x_numpy = x_sol.numpy()
                realh = hTrue.numpy()
                Gx = np.dot(G_numpy, x_numpy)
    #            print(Gx.shape)    # (43,)
                violateFactorTemp = []
                violateEdgeIndexTemp = []
                lenG = np.size(G_numpy, 0)
                for i in range(lenG):
                    if Gx[i] > realh[i]:
                        if realh[i] == 0:
                            realh[i] = 1e-3
                        newT = realh[i]/Gx[i]
                        violateEdgeIndexTemp.append(i)
                        violateFactorTemp.append(newT)
                violateFactor = 1
                violateEdgeIndex = -1
                if len(violateFactorTemp) > 0:
                    violateFactor = min(violateFactorTemp)
                    theMostViolatedIndex = violateFactorTemp.index(violateFactor)
                    violateEdgeIndex = violateEdgeIndexTemp[theMostViolatedIndex]
