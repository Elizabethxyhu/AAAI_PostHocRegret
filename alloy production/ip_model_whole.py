...
#                print(x_sol)
                ...
                x_numpy = x_sol.numpy()
                h_numpy = h.numpy()
                rowSizeG = np.size(realG_numpy, 0)
#                print(itemNum)
                global violateFactor
                violateFactor = 1
                violateEdgeIndex = -1
                realGx = np.dot(realG_numpy, x_numpy)
#                print("realGx: ", realGx, "h_numpy: ", h_numpy)
                tauTemp = []
                tauIdTemp = []
                for i in range(rowSizeG):
                    if -realGx[i] < -h_numpy[i]:
                        newT = -h_numpy[i] / -realGx[i]
                        tauTemp.append(newT)
                        tauIdTemp.append(i)
#                print(tauTemp)
                if len(tauTemp) > 0:
                    violateFactor = max(tauTemp)
                    violateFactorId = tauTemp.index(violateFactor)
                    violateEdgeIndex = tauIdTemp[violateFactorId]
