...
#    preG = np.zeros((rowSizeG, colSizeG))
#    cnt = 0
#    for i in range(rowSizeG):
#        for j in range(colSizeG):
#            realG[i][j] = realGTemp[cnt]
#            preG[i][j] = preGTemp[cnt]
#            cnt = cnt + 1
...
#        print(sol)
    ...
    #        print(Gx, objective)
        ...
        if len(tauTemp) > 0:
            tau = max(tauTemp)
#            objective = objective + np.dot(np.multiply(c, 1+ip_model_wholeFile.penalty), np.multiply(sol, tau-1))
            objective = objective + np.dot(np.multiply(c, 1+penalty), np.multiply(sol, tau-1))
...
#                print(G_torch)
#                print(op_torch)
                x = IPOfunc(A=A_torch, b=b_torch, h=-h_torch, c=c_torch, GTrue=-G_torch, penalty=penalty_torch, max_iter=self.max_iter, thr=self.thr, damping=self.damping,
...
stopCriterion = 0
if penaltyTerm == 0 or penaltyTerm == 0.25:
    stopCriterion = 100
elif penaltyTerm == 0.5 or penaltyTerm == 1:
    stopCriterion = 120
else:
    stopCriterion = 170
...
#    c_test = np.loadtxt('./CombOptNet/test_pricesAll.txt')
#    x_test = np.loadtxt('./CombOptNet/test_featureAll.txt')
#    y_test = np.loadtxt('./CombOptNet/test_weightsAll.txt')
...
#    lr = 5e-7
    ...
    for j in range(3):
        clf = Intopt(c_train, h_train, A_data, b_data, penalty_train, damping=damping, lr=lr, n_features=featureNum, thr=thr, epochs=10)
...
#        predTrainVal = predTrainVal.detach().numpy()
#        predValue = np.zeros((predTrainVal.size, 3))
#        for i in range(predTrainVal.size):
#            predValue[i][0] = int(i/(rowSizeG*colSizeG))
#            predValue[i][1] = value_train[i]
#            predValue[i][2] = predTrainVal[i]
#    #    np.savetxt('./CombOptNet/NN_weights/NN_weights.txt', predValue, fmt="%.2f")
#        np.savetxt('./NN/NN(' + str(testi) + ').txt', predValue, fmt="%.2f")
...
#    recordBest[0][i] = np.sum(val_rslt[1])
    ...
    clfBest = Intopt(c_test, h_test, A_data, b_data, penalty_test, damping=damping, lr=lr, n_features=featureNum, thr=thr, epochs=4)
...
#    np.savetxt('./CombOptNet/NN_weights/NN_weights.txt', predValue, fmt="%.2f")
    np.savetxt('./data/brass/proposed/proposed_penalty' + str(penaltyTerm) + '(' + str(testi) + ').txt', predValue, fmt="%.2f")
