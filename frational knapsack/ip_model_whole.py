...
def IPOfunc(A =None,b =None,h=None,cTrue=None,GTrue=None,penalty=None,alpha0=0.9995,beta=0.1,pc = True,
...
#            time_start=time.time()
            ...
            if not complete:
                ...
                #correction
                realG_numpy = GTrue.numpy()
                x_numpy = x_sol.numpy()
                h_numpy = h.numpy()
                itemNum = np.size(realG_numpy, 0) - 1
#                print(itemNum)
                global violateFactor
                violateFactor = 1
                violateEdgeIndex = -1
                realGx = np.dot(realG_numpy[itemNum], x_numpy)
                if realGx > h_numpy[itemNum]:
                    violateFactor = h_numpy[itemNum] / realGx
                    violateEdgeIndex = itemNum
                    x_numpy = np.multiply(x_numpy, violateFactor)
                
                x_sol = torch.from_numpy(x_numpy)
                end = time.time()
                run_time += end -start
#                print("violateFactor: ", violateFactor, "violateEdgeIndex: ", violateEdgeIndex)
                ...
                if method==1: # full
                    ...
                    ctx.save_for_backward(x_v,y_v,t_v,
                        ...
                        c_v, A_v, b_v, A, b, G, h,
                        x_sol,
                        torch.tensor(violateFactor,dtype=torch.float),
                        torch.tensor(violateEdgeIndex,dtype=torch.int))
...
#            print('Solve LP cost:',time_end1-time_start,'s')
        ...
        def backward(ctx,del_x):
            ...
            nonlocal run_time
            start = time.time()
            x,y,t,tau,kappa,c,A,b,trueA,trueb,trueG,trueh,x_sol,violateFactor,violateEdgeIndex = ctx.saved_tensors
#            print(x_sol,violateFactor,violateEdgeIndex)
...
#                print(constantItem)
                ...
                for i in range(n_var):
                    for j in range(n_var):
                        corrGradTemp[i][j] = -x_numpy[i]*violateG[j]*constantItem
#                print(violateG.shape, corrGradTemp.shape)
                E = np.eye(n_var)
                violateFactorE = E * violateFactor
                corrGrad = corrGradTemp + violateFactorE    # Lemma1
#                print(corrGrad.shape)  # (48, 48)
#                dxh = np.dot(corrGrad, dxh)
                corrdxG = np.dot(corrGrad, dxG)
#                dxG = np.dot(1+penalty-penalty/violateFactor, dxG)
                dxG = np.dot(1+penalty, corrdxG) + np.dot(penalty, dxG)
                corrdxc_numpy = np.dot(corrGrad, dxc_numpy)
#                dxc_numpy = np.dot(1+penalty-penalty/violateFactor, dxc_numpy)
                dxc_numpy = np.dot(1+penalty, corrdxc_numpy) + np.dot(penalty, dxc_numpy)
#                for i in range(n_var):
